...
from model_status import ModelStatusIndicator
...
def main():
    ...
    try:
        # Create a temporary AI client to check status
        from ai_grader import AIGrader
        temp_ai_grader = AIGrader(grader)
        ModelStatusIndicator.show_model_status(temp_ai_grader.local_ai)
    except Exception as e:
        st.sidebar.error(f"Model status unavailable: {str(e)[:50]}...")
        st.sidebar.info("💡 Model will load when needed for grading")
