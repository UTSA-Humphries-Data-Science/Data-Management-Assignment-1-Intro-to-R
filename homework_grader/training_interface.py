...
class TrainingInterface:
    ...
    def get_submissions_for_review(self, assignment_filter, status_filter):
        ...
        query = """
            ...
            LEFT JOIN submissions s ON td.cell_content = s.notebook_path
        ...
        if status_filter == "Needs Review":
            query += " AND td.human_score IS NULL"
        elif status_filter == "Already Corrected":
            query += " AND td.human_score IS NOT NULL"
    ...
    def show_submission_review(self, submission):
        ...
        with st.expander(f"📋 {submission['assignment_name']} - {submission.get('student_name', 'Unknown')}"):
            ...
            with col1:
                ...
                st.write(f"Score: {submission['ai_score']}")
    ...
    def save_correction(self, training_id, score, feedback):
        """Save instructor correction to training data"""
        conn = sqlite3.connect(self.grader.db_path)
        cursor = conn.cursor()
        
        cursor.execute("""
            UPDATE ai_training_data
            SET human_score = ?, human_feedback = ?, corrected_at = ?
            WHERE id = ?
        """, (score, feedback, datetime.now().isoformat(), training_id))
